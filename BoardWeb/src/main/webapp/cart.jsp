...
<body>
                    ...
                    <div class="num">
                        <div class="updown">
                            <input type="text" name="p_num1" id="p_num1" size="2" maxlength="4" class="p_num" value="2"
                                onkeyup="javascript:basket.changePNum(kw,pnm);">
                            <span onclick="javascript:basket.changePNum('up',p_num1);">
                                <i class="fas fa-arrow-alt-circle-up up"></i> </span>
                            <span onclick="javascript:basket.changePNum('dw',p_num1);">
	...
	<script>
	// 도화지 지정
	let data = document.querySelector('.basketdiv');
	...
	str+= `<div class="row data" data-id="`;
	str+=item.itemId+'"';
	str+=`><div class="subdiv">
     ...
     str+=item.image+'"';
     str+=` width="60"></div>
        <div class="pname"> <span>`;
        
     str+=item.itemName+"</span> </div></div>";
    
     str+=`<div class="subdiv">
        <div class="basketprice">
            <input type="hidden" name="p_price" id="p_price1" class="p_price" value=`;
     str+= item.price+">";
     str+=item.price+"원</div>";
     str+=`
        <div class="num">
            <div class="updown">
                <input type="text" name=`;
    str+="p_num"+item.itemId;
    str+=`id="p_num"+item.itemId+" size="2" maxlength="4" class="p_num" value="`;
    str+=item.qty+'" ';
    str+=`onkeyup="javascript:basket.changePNum(kw, pnm);">
          <span onclick="javascript:basket.changePNum('up',`;
          str+=`item.itemId);">
              <i class="fas fa-arrow-alt-circle-up up"></i> </span>
          <span onclick="javascript:basket.changePNum('dw',`;
          str+=`item.itemId`;
          str+=`");">
              <i class="fas fa-arrow-alt-circle-down down"></i> </span>
          </div>
        ...
        <div class="sum">`;
    let cnt = document.querySelector('input.p_num${item.itemId}').value;
    str+=item.price*cnt+"원</div></div>";
    str+=`
    <div class="subdiv">
        <div class="basketcmd">
            <a href="javascript:void(0)" class="abutton" onclick="javascript:basket.delItem();">삭제</a>
